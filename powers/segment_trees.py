"""
Module with segment trees (statistic trees). A segment tree contains nodes in which each node
holds a statistic info that aggregates info of some intervals (segments) of a list of values.

Such tree can be built in O(NlogN) and allows querying info from intervals in O(logN + K)
